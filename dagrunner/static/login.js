...
async function sha256(value) {
  ...
  throw new Error(t("当前页面无法安全计算密码摘要，请使用 HTTPS 或在本机访问"));
}
...
function showCountdown(seconds) {
  ...
  const render = () => {
    ...
    errorBox.textContent = t("此 IP 已暂时禁止登录，请在 {time} 后重试", {time:`${minutes}:${secs}`});
    button.disabled = true;
    if (remaining <= 0) {
      clearInterval(countdownTimer);
      countdownTimer = null;
      errorBox.textContent = t("可以重新尝试登录");
...
form.addEventListener("submit", async event => {
  ...
  try {
    ...
    const response = await fetch("/api/auth/login", {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-DAGRunner-Language": currentLanguage },
      body: JSON.stringify({
    ...
    if (!response.ok) {
      const suffix = Number.isInteger(result.remaining_attempts)
        ? t("，还可尝试 {count} 次", {count:result.remaining_attempts})
        : "";
      throw new Error(`${t(result.error || "登录失败")}${suffix}`);
    }
    window.location.replace(result.redirect || "/");
  } catch (error) {
    errorBox.textContent = error.message || t("登录失败");
  } finally {
    password.value = "";
